...
def run(paths) -> tuple:
    ...
    order_payments_validator = OrderPaymentsValidator(order_payments)
    order_payments_cleaner = OrderPaymentCleaner(order_payments)
    ...
    payment_order_ids = Loader.load_customers(BASE_DIR / paths["order_payments"], column = ["order_id"])["order_id"]
    ...
    review_order_ids = Loader.load_customers(BASE_DIR / paths["order_reviews"], column = ["order_id"])["order_id"]
    ...
    order_ids = Loader.load_customers(BASE_DIR / paths["orders"], column = ["order_id"])["order_id"]
    ...
    order_items_order_ids = Loader.load_customers(BASE_DIR / paths["order_items"], column = ["order_id"])["order_id"]
    product_order_ids = orders_rule.get_invalid_order_ids_from_invalid_references(order_items, product_ids)
    return product_order_ids, order_items_order_ids, review_order_ids, payment_order_ids